...
app.post('/products', (req, res) => {
  ...
  if (typeof name !== 'string' || typeof price !== 'number' || typeof inStock !== 'boolean') {
    return res.status(400).json({ error: 'Invalid product format. Must include name (string), price (number), and inStock (boolean).' });
  }
...
app.delete('/products/:id', (req, res) => {
  ...
  writeProducts(products);
  res.json({ message: `Product with id ${id} deleted successfully.`, deleted });
});
